...
module type DictionarySig = sig
  ...
  sig type t val compare : t -> t -> order end
  module type Formattable =
  sig type t val format : Format.formatter -> t -> unit end
  module type KeySig =
  sig
    type t
...
module type DictionarySetSig = sig
  ...
  module Make : functor (E : ElementSig) 
    -> functor (DM : Dictionary.DictionaryMaker) 
